module.exports = {
	...
	commandFunction: function (Bot, room, time, by, args, client) {
		...
		if (DBU.hostBanned || DBU.raidBanned) return Bot.roomReply(room, by, `You are banned from hosting raids - please contact a roomstaff member for further details.`);
		...
		if (args.find(fullArg => {
			...
			if (/^hostingin/.test(arg)) {
				arg = toID(arg).substr('hostingin'.length);
				...
				if (timeHost) return Bot.roomReply(room, by, `Uhh why are you hosting at two times again\nIs this what they call two-timing?`) || true;
				timeHost = timeHostRun;
				return false;
			}
			if (arg.startsWith('note')) {
				note = fullArg.replace(/note\W*/i, '');
				if (note === fullArg) return Bot.roomReply(room, by, `C-could you please not add random characters in the note?`) || true;
				return false;
			}
			const timeTry = tools.fromHumanTime(arg);
			if (timeTry) {
				if (timeLeft) return Bot.roomReply(room, by, `...it's expiring twice? Are you sure you didn't mean to host at one of those instead? If so, add a 'hosting in' before that time!`) || true;
		...
		if (timeLeft > 4 * 60 * 60 * 1000) return Bot.roomReply(room, by, `Ah, yes. I, too, enjoy hosting raids that last for ${tools.toHumanTime(timeLeft)}`);
		const user = toID(by);
		if (!Bot.rooms[room].raids) Bot.rooms[room].raids = {};
		if (Bot.rooms[room].raids[user]) return Bot.rooms[room].raids[user].HTML();
		...
		raid.host = user;
		raid.pings = (raid.wb ? Bot.rooms[room].users.filter(u => DB.get(toID(u))?.raids.hasOwnProperty(raid.mon)) : Bot.rooms[room].users.filter(u => DB.get(toID(u))?.raids[raid.mon] === false)).map(u => u.substr(1));
		raid.players = {};
		raid.slots = slots;
		...
		raid.HTML = function (pings) {
			...
			let joinHTML = players.length > 1 ? `<br/><table style="border:1px;border-collapse:collapse;"><tr><th style="width:100px;">IGN</th><th style="width:30px;">Lv</th><th style="width:120px;">Friend Code</th></tr>${players.map(u => `<tr><td>${tools.escapeHTML(u.ign)}</td><td>${u.level}</td><td>${u.code.slice(0, 4)}<span style="padding:5px;">${u.code.slice(4, 8)}</span>${u.code.slice(8, 12)}</td></tr>`).join('')}</table><br/>` : '-<br/><br/>';
			...
			let hiddenCodes = joinHTML.replace('<th style="width:120px;">Friend Code</th>', '').replace(/<td>\d{4}<span style="padding:5px;">\d{4}<\/span>\d{4}<\/td>(?=<\/tr)/g, '');
		...
		if (timeHost) {
			...
			raid.hostStamp = `Hosting at <b>XX:${new Date(raid.hostTime).toLocaleTimeString('GMT', {timeZone: 'GMT'}).split(':')[1]}</b>`;
