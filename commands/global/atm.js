	...
	commandFunction: function (Bot, room, time, by, args, client) {
		...
		if (!Bot.rooms[room].lb) out = 'This room doesn\'t have a Leaderboard.';
			...
			if (Bot.rooms[room].lb.users[user].points.reduce((a, b) => a || b, false)) {
				out = ` ${Bot.rooms[room].lb.users[user].name}'s points: ${Bot.rooms[room].lb.users[user].points.filter((p, i) => p > 0 && Bot.rooms[room].lb.points[i]).map((p, i) => p + ' ' + Bot.rooms[room].lb.points[i][2]).join(', ')}.`;
			}
