	...
	commandFunction: function (Bot, room, time, by, args, client, isPM) {
		...
		switch (args[0].toLowerCase()) {
			...
			case 'new': case 'n': case 'n.n': case 'nwn': {
				...
				if (isPM) return Bot.pm(by, "This command is only usable in the chatroom.");
			...
			case 'premove': {
				...
				let squares = args.join('').toLowerCase().split('-');
				if (squares.length !== 2) return Bot.pm(by, `OMA A ${squares.length}-SQUARE PREMOVE! OUTSTANDING!`);
				...
				fs.readdir('./data/BACKUPS', (err, files) => {
					...
					if (games.length) {
						Bot.say(room, `/adduhtml CHESSBACKUPS, <details><summary>Game Backups</summary><hr />${games.map(game => {
							...
							return `<button name="send" value="/msg ${Bot.status.nickName},${prefix}chess ${room} restore ${game}">${info.W.name} vs ${info.B.name}</button>`;
						}).join('<br />')}</details>`);
					}
			...
			case 'restore': case 'resume': case 'r': {
				...
				if (!id) return Bot.pm(by, "Invalid ID.");
				if (Bot.rooms[room].chess?.[id]) return Bot.pm(by, "Sorry, that game is already in progress!");
				fs.readFile(`./data/BACKUPS/chess-${room}-${id}.json`, 'utf8', (err, file) => {
					if (err) return Bot.pm(by, "Sorry, couldn't find that game!");
					if (!Bot.rooms[room].chess) Bot.rooms[room].chess = {};
			...
			case 'emote': case 'emoticon': case 'unicode': case 'ascii': {
				if (!Bot.rooms[room].chess) return Bot.pm(by, 'This room does not have any chess games.');
				if (!tools.hasPermission(by, 'gamma', room)) return Bot.pm(by, 'Access denied.');
				args.shift();
				let id;
			...
			case 'resign': case 'forfeit': case 'f': case 'ff': case 'ihavebeenpwned': case 'bully': {
				...
				if (!id) return Bot.pm(by, "Unable to find the game you meant to resign in.");
				let game = Bot.rooms[room].chess[id];
				if (!game) return Bot.pm(by, "Not a valid ID.");
			...
			case 'offerdraw': case 'draw': case 'd': case 'offertie': case 'tie': case 'handshake': case 'accept': case 'acceptdraw': {
				...
				if (!id) return Bot.pm(by, "Unable to find the game you meant.");
				let game = Bot.rooms[room].chess[id];
				if (!game) return Bot.pm(by, "Not a valid ID.");
				if (!game.started) return Bot.pm(by, '>drawing before it starts');
				if (![game.W.player, game.B.player].includes(user)) return Bot.pm(by, "Only a player can draw.");
				const side = game.W.player === user ? "W" : "B";
			...
			case 'spectate': case 's': case 'watch': case 'w': {
				...
				let user = toId(by);
				if (game.spectators[user]) return Bot.pm(by, `You're already spectating! If you want to stop, use \`\`${prefix}chess unspectate\`\` instead!`);
			...
			case 'unspectate': case 'us': case 'unwatch': case 'uw': case 'zestoflifeisstinky': {
				...
				if (!game.spectators[user]) return Bot.pm(by, `You aren't spectating! If you want to, use \`\`${prefix}chess spectate\`\` instead!`);
			...
			case 'flip': case 'flipboard': case 'switch': case 'switchsides': case 'rotate': {
				...
				let game = Bot.rooms[room].chess[id];
				if (!game) return Bot.pm(by, "Sorry, no chess game here to flip.");
				if (!game.started) return Bot.pm(by, "AYAYAYA - no.");
				let user = toId(by);
				if ([game.W.player, game.B.player].includes(user)) return Bot.pm(by, "Imagine trying to play as the opponent.");
				if (!game.spectators[user]) return Bot.pm(by, `You aren't spectating! If you want to, use \`\`${prefix}chess spectate\`\` first!`);
				game.spectators[user] = game.spectators[user] === 'W' ? 'B' : 'W';
				Bot.pm(by, "Flipped le board!");
				break;
			}
			case 'rejoin': case 'rj': {
				...
				ids.forEach(id => {
					...
					if (!side && !game.spectators[user]) return Bot.pm(by, `You don't look like a player / spectator - try ${prefix}chess spectate ${id}... ;-;`);
			...
			case 'fen': case 'view': case 'v': case 'show': case 'display': case 'position': {
				if (!tools.hasPermission(by, 'beta', room) && (room !== 'boardgames' || !tools.hasPermission(by, 'gamma', room))) return Bot.pm(by, "Access denied.");
