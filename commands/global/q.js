module.exports = {
    ...
    switch (Bot.rooms[room].rank) {
      case '*': case '#': case '&': case '~': {
        switch (exec) {
          case 'add': {
            if (!tools.hasPermission(by, 'beta')) return Bot.say(room, 'Access denied.');
            args.shift();
          ...
          case 'preview': {
            ...
            let addstr = '';
            inite.forEach(line => {
              addstr += tools.quoteParse((/^ /.test(line))?(line.substr(1)):(line));
            });
            if (tools.hasPermission(by, 'gamma')) return Bot.say(room, '/adduhtml QUOTE, <HR>' + addstr + '<HR>');
            else return Bot.say(room, '/pminfobox ' + by + ', ' + addstr);
            break;
          }
          case 'list': {
            if (!tools.hasPermission(by, 'gamma')) return Bot.say(room, 'Access denied.');
                if (!fs.readdirSync('./data/QUOTES').includes(room + '.txt')) return Bot.say(room, 'No quotes for this room.');
                let qArr = JSON.parse(fs.readFileSync('./data/QUOTES/' + room + '.txt', 'utf8'));
                if (!qArr.length) return Bot.say(room, 'No quotes for this room.');
            let outstr = '';
            qArr.forEach(quote => {
              outstr += ('<DETAILS><SUMMARY>Q' + (qArr.indexOf(quote) + 1) + ': </SUMMARY><HR>' + tools.quoteParse(quote) + '<HR></DETAILS><BR>');
            });
          ...
          case 'delete': {
            if (!tools.hasPermission(by, 'beta')) return Bot.say(room, 'Access denied.');
                if (!fs.readdirSync('./data/QUOTES').includes(room + '.txt')) return Bot.say(room, 'No quotes in this room.');
            if (!args[1]) return Bot.say(room, unxa);
                let qArr = JSON.parse(fs.readFileSync('./data/QUOTES/' + room + '.txt', 'utf8'));
                if (!qArr.length) return Bot.say(room, 'No quotes for this room.');
            args.splice(0, 1);
            let gno = parseInt(args.join(' ').replace(/[^0-9]/g, ''));
            if (isNaN(gno) || gno > qArr.length || gno < 1) return Bot.say(room, 'Invalid quote number.');
          ...
          default: {
            ...
            if (isNaN(ano) || ano < 1 || ano > qArr.length) return Bot.say(room, 'Invalid quote number.');
            let qtext = tools.quoteParse(qArr[ano - 1]);
            if (tools.hasPermission(by, 'gamma')) return Bot.say(room, '/adduhtml QUOTE, <HR>' + qtext + '<HR>');
      ...
      case '+': case '%': case '@': case '-': case '$': case '★': {
        switch (exec) {
          ...
          case 'amt': case 'amount': {
            ...
            let qArr = JSON.parse(fs.readFileSync('./data/QUOTES/' + room + '.txt', 'utf8'));
            if (!qArr.length) return Bot.say(room, 'No quotes for this room.');
            if (tools.hasPermission(by, 'gamma')) return Bot.say(room, 'This room has ' + qArr.length + ' quote' + (qArr.length > 1 ? 's' : '') + '.');
          ...
          default: {
            ...
            if (tools.hasPermission(by, 'gamma')) {
              if (qtext.length > 290) return Bot.say(room, 'Q' + ano + ' is too long for me to post, sorry.');
              return Bot.say(room, '[[]][[]]' + qtext);
            }
            else if (qtext.length > 290) return Bot.pm(by, 'Q' + ano + ' is too long for me to send, sorry.');
      ...
      case ' ': {
        switch (exec) {
          ...
          case 'list': {
            return Bot.say(room, 'I do not have the required auth to display the quote list. Mind promoting me, first?');
            break;
          }
          case 'delete': {
            if (!tools.hasPermission(by, 'beta')) return Bot.say(room, 'Access denied.');
