	...
	commandFunction: function(Bot, room, time, by, args, client, pm) {
		...
		switch (exec) {
			case "add": {
				...
				let addstr = args.join(" ").replace(/ ?\\n ?/g, '\n');
				let qArr = JSON.parse(fs.readFileSync("./data/QUOTES/" + room + ".json", "utf8"));
				qArr.push(addstr);
				fs.writeFile("./data/QUOTES/" + room + ".json", JSON.stringify(qArr, null, '\t'), e => {
			...
			case "list": {
				...
				if (!fs.readdirSync("./data/QUOTES").includes(room + ".json")) return Bot.pm(by, "No quotes for this room.");
				let qArr = JSON.parse(fs.readFileSync("./data/QUOTES/" + room + ".json", "utf8")), outstr = '';
			...
			case "delete": {
				if (pm || !tools.hasPermission(by, "beta", room)) return Bot.pm(by, "Access denied.");
				if (!fs.readdirSync("./data/QUOTES").includes(room + ".json")) return Bot.pm(by, "No quotes in this room.");
