module.exports = {
  ...
  commandFunction: function (Bot, room, time, by, args, client) {
    ...
    if (/[a-zA-Z]/.test(cargs[0])) {
      ...
      tools.addPoints(cargs[0], points, room);
      return Bot.say(room, `${cargs[0]} was given ${points} ${points == 1 ? Bot.rooms[room].shop.points[0] : Bot.rooms[room].shop.points[1]}.`);
    }
    else if (/[a-zA-Z]/.test(cargs[1])) {
      let points = parseInt(cargs[0].replace(/[^0-9-]/g, ''));
      if (isNaN(points)) return Bot.say(room, 'Invalid points.');
      tools.addPoints(cargs[1], points, room);
      return Bot.say(room, `${cargs[1]} was given ${points} ${points == 1 ? Bot.rooms[room].shop.points[0] : Bot.rooms[room].shop.points[1]}.`);
    }
