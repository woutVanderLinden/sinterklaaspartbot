			...
			case '★': {
				switch (exec) {
					case "add": {
						...
						let qArr = JSON.parse(
							fs.readFileSync("./data/QUOTES/" + tcroom + ".json", "utf8")
						);
						qArr.push(addstr);
						fs.writeFile(
							"./data/QUOTES/" + tcroom + ".json",
					...
					case "delete": {
						if (!tools.hasPermission(by, "beta", room))
							return Bot.say(room, "Access denied.");
						if (!fs.readdirSync("./data/QUOTES").includes(room + ".json"))
					...
					case "amt":
					case "amount": {
						if (!fs.readdirSync("./data/QUOTES").includes(tcroom + ".json"))
							return Bot.say(room, "No quotes in this room.");
						let qArr = JSON.parse(
							fs.readFileSync("./data/QUOTES/" + tcroom + ".json", "utf8")
						);
			...
			case "★": {
				switch (exec) {
					case "add": {
						...
						if (!fs.readdirSync("./data/QUOTES").includes(room + ".json")) {
							fs.writeFileSync("./data/QUOTES/" + room + ".json", "[]");
						}
						let qArr = JSON.parse(
							fs.readFileSync("./data/QUOTES/" + room + ".json", "utf8")
						);
						qArr.push(addstr);
						fs.writeFile(
							"./data/QUOTES/" + room + ".json",
							JSON.stringify(qArr, null, 2),
							e => {
								if (e) return Bot.log(e);
								Bot.say(room, "Quote has been added.");
