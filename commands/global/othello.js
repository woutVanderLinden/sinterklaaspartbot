...
function runMoves (run, info, game) {
	...
	switch (run) {
		...
		case 'join': {
			...
			let html = `<hr /><h1>Othello Signups are active!</h1>${["W", "B"].filter(side => !game[side].player).map(side => `<button name="send" value="/botmsg ${Bot.status.nickName}, ${prefix}othello ${room}, join ${id} ${side === 'W' ? 'White' : 'Black'}">${side === 'W' ? 'White' : 'Black'}</button>`).join('&nbsp;')}<hr />`;
	...
	commandFunction: function (Bot, room, time, by, args, client, isPM) {
		...
		switch (args[0].toLowerCase()) {
			...
			case 'spectate': case 's': case 'watch': case 'w': {
				...
				return Bot.roomReply(room, by, `You are now spectating the Othello match between ${game.W.name} and ${game.B.name}!`);
				break;
			}
			case 'unspectate': case 'us': case 'unwatch': case 'uw': case 'zestoflifeisstinky': {
				...
				if (!game.spectators.includes(user)) return Bot.roomReply(room, by, `You aren't spectating! If you want to, use \`\`${prefix}othello spectate\`\` instead!`);
				game.spectators.remove(user);
				return Bot.roomReply(room, by, `You are no longer spectating the Othello match between ${game.W.name} and ${game.B.name}.`);
				break;
			}
			case 'rejoin': case 'rj': {
				...
				ids.forEach(id => {
					...
					if (!side && !game.spectators.includes(toID(by))) return Bot.roomReply(room, by, `You don't look like a player / spectator - try ${prefix}othello spectate ${id}... ;-;`);
