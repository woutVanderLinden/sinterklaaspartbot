class game {
	...
	switchPick (forced) {
		if (!this.enemy) return this.ai === 0 ? this.side.pokemon.map((blank, i) => i + 1).filter(i => this.side.pokemon[i - 1].condition.includes('/') && !this.side.pokemon[i - 1].active).random() : this.side.pokemon.filter(mon => mon.condition.includes('/') && !mon.active).random().ident.split(': ').slice(1).join(': ');
		switch (this.ai) {
			case 0: {
				if (forced) return this.side.pokemon.map((blank, i) => i + 1).filter(i => this.side.pokemon[i - 1].condition.includes('/') && !this.side.pokemon[i - 1].active).random();
				if ((!forced && Math.random() > 0.1) || (this.noSwitch)) return false;
				let opts = this.side.pokemon.map((blank, i) => i + 1).filter(i => this.side.pokemon[i - 1].condition.includes('/') && !this.side.pokemon[i - 1].active);
