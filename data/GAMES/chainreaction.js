class CR {
	...
	isAlive (player) {
		...
		for (let i = 0; i < this.height; i++) for (let j = 0; j < this.width; j++) if (this.board[i][j].value && this.board[i][j].col == col) return true;
		return false;
	}
	willExplode (board) {
		if (!board) board = this.board;
		for (let i = 0; i < this.height; i++) for (let j = 0; j < this.width; j++) if (board[i][j].value >= board[i][j].crit) return true;
		return false;
	}
	detonate (board) {
		if (!board) board = this.board;
		...
		for (let i = 0; i < this.height; i++) for (let j = 0; j < this.width; j++) if (clone[i][j].value >= clone[i][j].crit) {
