module.exports = {
	...
	commandFunction: function (args, message, Bot) {
		if (!message.member.roles.cache.find(role => role.name === 'Validator')) return message.channel.send('Access denied.').then(msg => msg.delete({timeout: 3000}));
			...
			Promise.all(replays.map(replay => {
				return new Promise((resolve, reject) => {
					axios.get(replay).then(response => {
						let players = response.data.match(/\n\|player\|p[12]\|[^|]+/g);
					...
					return new Promise((resolve, reject) => {
						try {
							axios.get(replay).then(res => {
								const data = res.data;
								let result = {id: parseInt(replay.split(/gen[78]1v1-/)[1].split('-')[0]), type: false, rating: [], LL: false};
								let input = data.split('\n'), inThis = null, enemy = false;
								for (let line of input) {
									...
									switch (args[1]) {
										...
										case 'poke': {
											...
											if (args[2] === inThis) {
												let species = args[3].split(', ')[0].split('-')[0];
												if (!output.team[species]) output.team[species] = {
													species: undefined,
													level: 100,
													moves: []
												}
												if (/^L\d{1,2}$/.test(args[3].split(', ')[1])) output.team[species].level = parseInt(args[3].split(', ')[1].substr(1));
											}
											break;
										}
										case 'rated': {
										...
										case 'switch': {
											...
											if (args[2].startsWith(inThis + 'a: ')) {
												output.team[args[3].split(', ')[0].split('-')[0]].species = args[3].split(', ')[0];
												output.team[args[3].split(', ')[0].split('-')[0]].nick = args[2].split(inThis + 'a: ')[1];
											}
											break;
										}
										case 'move': {
											...
											if (args[2].startsWith(inThis + 'a: ')) {
												if (output.team[args[2].split(inThis + 'a: ')[1]]) output.team[args[2].split(inThis + 'a: ')[1]].moves.push(args[3]);
												else Object.values(output.team).find(m => m.nick === args[2].split(inThis + 'a: ')[1]).moves.push(args[3]);
											}
											break;
										}
										case 'win': {
										...
										case 'raw': {
											let match = args.join('|').match(/^\|raw\|(.*?)'s rating: (\d{4}) &rarr; <strong>(\d{4})<\/strong><br \/>\([+-]\d{1,2} for (?:winn|los)ing\)\s*$/);
