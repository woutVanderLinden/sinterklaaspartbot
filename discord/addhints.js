...
module.exports = {
	...
	commandFunction: function (args, message, Bot) {
		...
		let team = PZ.getTeam(toID(cargs.shift())), amt = parseInt(cargs.join('').replace(/[^0-9-]/g, ''));
		...
		PZ.addHints(team, amt).then(nam => message.channel.send(`${amt} hint(s) added! ${team.name} now has ${nam} hint(s).`)).catch(e => Bot.log(e));
	}
}