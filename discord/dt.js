module.exports = {
	...
	commandFunction: function (args, message, Bot) {
		...
		const addr = ('000' + index).substr(-4).replace(/^0/, '');
		const Discord = require('discord.js');
		const embed = new Discord.MessageEmbed();
		embed.setAuthor(`#${('00' + mon.num).substr(-3)}`, `${websiteLink}/public/sprites/tile${addr}.png`);
		embed.setTitle(`${mon.name} (${mon.tier || `[OU]`})`);
		const statList = [...Object.values(mon.baseStats), Object.values(mon.baseStats).reduce((a, b) => a + b)];
		const statStr = statList.map(n => String(n).padStart(3, ' ')).map(n => ` ${n} `).join('|');
		...
		const imgLink = `https://play.pokemonshowdown.com/sprites/ani/${base + (forme ? `-${forme}` : '')}.gif`;
		embed.addFields(
